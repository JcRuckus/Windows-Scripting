...
TITLE Joshua Rucker's automagic PC setup and upgrade script
...
ECHO Script to Query Device Model from WMIC and move to section for that model detected
    ...
    if /i "%%~b"=="Latitude E5270" goto LatitudeE5470
    if /i "%%~b"=="Latitude E5470" goto LatitudeE5470
    if /i "%%~b"=="Latitude E5570" goto LatitudeE5470
    if /i "%%~b"=="Precision 3510" goto LatitudeE5470
...
REM The following command executes all files in folders and subfolders silently, one at a time and in order
	...
	ECHO Drivers and Firmware Complete. Continuing to Dell Command Update Installation
...
:LatitudeE5470
ECHO ===================================================================================E5x70
	ECHO Start of script for model [Latitude E5270-E5470-E5570 and Dell Precision 3510]
	ECHO.
	ECHO Switching to model specific drivers install folder...
	CD ".\Base Installs\Firmware and Drivers\Latitude E5270-E5470-E5570 and Dell Precision 3510"
	ECHO Installing Drivers...
...
ECHO :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
	...
	%windir%\syswow64\tzutil /s "Central Standard Time"
	ECHO.

ECHO ::::::::::::::::::::::::::::::::::Microsoft .NET Prerequisite Installation::::::::::::::::::::::::::::::
ECHO.
ECHO ~~~~~~~~~~~~~~~~~~~Installing .Net 35 on-demand files to local workstation~~~~~~~~~~~~~~~~~
	%windir%\syswow64\DISM.EXE /Online /Add-Package /PackagePath:"%~dp0base installs\Net\Microsoft-Windows-NetFx3-OnDemand-Package-en-us.cab"
ECHO.	
ECHO ~~~~~~~~~~~~~~~~~~~~~~~~~Offline install/Enablement of .NET 3.5~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	DISM /Online /Enable-Feature /FeatureName:NetFx3 /All /LimitAccess /Source:%~dp0sources\sxs
	
ECHO ~~~~~~~~~~~Installing Microsoft .NET 4.8 prerequisite for Dell Command Update~~~~~~~~~~~~~~
	"%~dp0base installs\Net\net48_offline_installer_ndp48-x86-x64-allos-enu.exe" /q /norestart
	TIMEOUT 45
ECHO.
ECHO ::::::::::::::::::::::::::::::::::::COMPLETED .NET Installs::::::::::::::::::::::::::::::::
ECHO.
ECHO.
CD "%~dp0base installs\firmware and drivers"
ECHO ~~~~~~~~~~~~~~Installing Dell Command Update Windows Universal v4 Silently~~~~~~~~~~~~~~~~~
	START Dell-Command-Update-Windows-Universal-Application_601KT_WIN_4.5.0_A00_01 -s
ECHO.
ECHO.
...
ECHO Installing all Docking Station Drivers
	...
	START DA100\DisplayLink-Dock-Driver_FF21H_WIN_10.1.2814.0_A04 -s
...
:SoftwareInstall
	...
	TIMEOUT 15
	START %windir%\system32\notepad.exe "%~dp0base installs\Ploview Password.txt"
	"C:\Program Files (x86)\Isoplotec\PloView\PloView.exe"
	...
	START eDrawingsAllX64 /qb /norestart
::edrawingsallx64.exe "msiexec /i edrawings.msi /qb /norestart"
	...
	START %windir%\system32\notepad.exe "%~dp0base installs\Freedom Software\TSTech-Hidalgo.txt"
	"C:\Program Files (x86)\ErrorProof\FTCRegistrySettings.exe"
	TIMEOUT 15
	Xcopy "c:\programdata\microsoft\windows\start menu\programs\errorproof\Administrator.lnk" c:\users\public\desktop
	Xcopy "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\ErrorProof\Windows Entry.lnk" c:\users\public\desktop
	CD..
	CD..
	CD..
	...
	TIMEOUT 30
	:: script to set adobe as pdf default for all users
	CD..
	...
	TIMEOUT 30
	START iexplore.exe http://svna187.tstech.com:8080/qadhome_eam/client/setup.html
	TIMEOUT 30
	ECHO.
	ECHO ===================================================================================Language Packs
:LanguagePacks
...
ECHO Remember to manually add Additional Languages from Windows Settings
...
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
::					Pending to script				      ::
::AcroRead.msi ADDLOCAL=ThumbnailPreviewHandler											      ::
:: Message to user:
	::do
...
:END
	ECHO Remember to manually add Additional Languages from Windows Settings
	ECHO Remember to Configure Adobe Acrobat Reader as the default PDF handler
